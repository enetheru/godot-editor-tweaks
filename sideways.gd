...
func _process_custom_fx( char_fx:CharFXTransform ) -> bool:
	...
	# REQUIRES CUSTOM GODOT: ts.font_get_x_height(rid, font_size)
	var x_height:float = ts.font_get_ascent(rid, font_size) * 0.56

	char_fx.transform *= Transform2D(
		deg_to_rad(-angle), Vector2(x_height * scale, 0))
	char_fx.transform = (
		char_fx.transform
		.translated(Vector2(0, dist.y * smush))
		.scaled_local(Vector2(scale, scale)))
	return true
...
static func sideways(
			...
			angle:float = 90,
			scale:float = 1.3,
			smush:float = 0.3 ) -> String:
	return (
		"[sideways angle=%f scale=%f smush=%f]" % [angle, scale, smush]
